...
# Update time window for 8-week cohort
START_TIME = datetime(2025, 9, 1, 0, 0, 0, tzinfo=timezone.utc)
END_TIME = datetime(2025, 10, 26, 23, 59, 59, tzinfo=timezone.utc)
...
def resolve_first_block_at_or_after(
    ...
    print(f"[{chain_name}] Starting binary search for start block (Target TS: {target_timestamp})")

    while low < high:
        mid = (low + high) // 2
        timestamp = get_block_timestamp(w3, mid)

        print(f"  -> [Start] Range: [{low} - {high}] | Mid: {mid} | Timestamp: {timestamp}")
    ...
    final_timestamp = get_block_timestamp(w3, low)
    if final_timestamp < target_timestamp:
        raise ValueError("No block found at or after target timestamp.")

    print(f"[{chain_name}] Resolved start block: {low} (TS: {final_timestamp})\n")
    return low
...
def resolve_last_block_at_or_before(
    ...
    print(f"[{chain_name}] Starting binary search for end block (Target TS: {target_timestamp})")

    while low < high:
        mid = (low + high + 1) // 2
        timestamp = get_block_timestamp(w3, mid)

        print(f"  -> [End] Range: [{low} - {high}] | Mid: {mid} | Timestamp: {timestamp}")
    ...
    print(f"[{chain_name}] Resolved end block: {low} (TS: {final_timestamp})\n")
    return low
...
def save_manifest(manifest: dict) -> Path:
    ...
    # Output file path
    output_path = MANIFEST_DIR / "block_range_8w.json"
...
def main() -> None:
    ...
    if not all([eth_rpc_url, arb_rpc_url, base_rpc_url]):
        raise RuntimeError("ETH_RPC_URL, ARB_RPC_URL, or BASE_RPC_URL is not configured.")
    ...
    base_w3 = create_web3(base_rpc_url)

    start_ts = int(START_TIME.timestamp())
    end_ts = int(END_TIME.timestamp())

    print("\n--- Resolving Ethereum block range ---")
    ethereum_range = resolve_chain_range(eth_w3, start_ts, end_ts, "Ethereum")

    print("--- Resolving Arbitrum block range ---")
    arbitrum_range = resolve_chain_range(arb_w3, start_ts, end_ts, "Arbitrum")

    print("--- Resolving Base block range ---")
    base_range = resolve_chain_range(base_w3, start_ts, end_ts, "Base")

    manifest = build_manifest(ethereum_range, arbitrum_range, base_range)
    manifest_path = save_manifest(manifest)

    print("=" * 60)
    print("Block Resolution Complete (8-Week Cohort)")
    print("=" * 60)
    ...
    for name, block_range in (
        ...
        print(f"Start    : {block_range.start.number} ({block_range.start.hash})")
        print(f"End      : {block_range.end.number} ({block_range.end.hash})")
